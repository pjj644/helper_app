...
## 2026-08: 学期 ID 映射错位修复（后端配对修正 + 端侧语义一致性闸门）

- **根因**：后端 app-config 下发了自相矛盾的配对 `(baseSemesterId=503, semesterLabel='2026-2027-1')`，而教务真实 id 为 2025-2026-2=503、2026-2027-1=523（步进 20）；端侧 `resolveBaseSemesterMapping` 将 id+标签锚点成对当映射基准，导致全部学期 id 偏移 -20——用户选「2026-2027-1」下载课表实际抓了 503（2025-2026-2）的数据（真机日志实锤：目标学期=503）；`parseSemesterConfigJson` 原本只校格式不校语义，坏配置被全盘接受；
- **后端修正**（ai-proxy 独立仓 `fc718ff`）：`src/config/appConfig.json` 的 `baseSemesterId` 503 → 523 与标签重新配对；配置有进程内缓存，需重启后端生效；
- **端侧语义闸门**：`SemesterConfig.parseSemesterConfigJson` 新增一致性校验——标签可解析时按固定历史锚点（483↔2025-2026-1，教务 id 空间客观事实）重算期望 id，与 `baseSemesterId` 不符则整包拒绝回落兜底 (503↔2025-2026-2)，两条路径均收敛到当前学期 523；存量已缓存的坏配置升级后被闸门自动拦下自愈；标签为空时跳过校验保持向后兼容；
- **成绩默认上一学期策略未受影响（硬约束）**：`getPreviousSemesterId() = calculateSemesterId() - 20` 相对逻辑未动；修复后成绩默认为 503（2025-2026-2，正确上一学期；坏配置下曾被错算为 483）；
- **Review 结论（独立子代理）**：无阻塞问题——端云两侧修复互为兜底、发布无顺序依赖；学期选项/标签反解/考试/成绩全消费方同口径一致；全仓无其他硬编码学期 id；已知限制：坏配置窗口期写入的 503 分片若实际是 2026-2027-1 数据，属既有污染，用户重抓对应学期即可净化；
- **验证**：`devecocli build` BUILD SUCCESSFUL + `devecocli check lint` 0 error（51 条预存在 warning 与基线一致）+ 后端 `npm run typecheck` 通过；学期推演（2026-08-30）：当前学期=523、成绩默认=503、标签 2025-2026-2→503、2026-2027-1→523。
