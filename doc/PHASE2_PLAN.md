...
> **版本**：v2.2（2026-08-26）　**状态**：✅ 全部任务已实施（P2-7a/b/c/9 与 7d 均合入），待 🔴 用户侧验收（§5）
...
| P2-3 | 云同步升级 | ✅ 完成（P2-7a/b/c/d 补完，见 §2） | `a043aaf` → `683fd74` |
| P2-4 | 晨报卡片 | ❌ 移出本轮 | — |
...
| P2-7d | AI 会话加密上云（仅会话，日程不纳入） | P1 | 大 | 7a/7b 合入 | ✅ `a1cce25`+`683fd74` |
...
### P2-7d AI 会话加密上云 `[已完成 · schema a1cce25 + 端侧 683fd74]`

> 实现要点（与上文设计的差异）：存储粒度为**每会话一条记录**（ChatSession 是自包含 blob，原按消息拆表的设想不必要）；主键直接复用端侧 `session.id`（本地 UUID，幂等 upsert）。密钥派生 PBKDF2-HMAC-SHA256×10000（UID+固定盐），AES-256-GCM，payload = base64(IV‖密文‖tag)；AAD 绑定应用语境。上传后裁剪云端多余记录使 20 条上限语义尽力传播；下载合并经无钩子内部通道回写防循环；单会话明文 >1.5MB 跳过上传（防超 CloudDB 单记录 2MB 上限）。已知限制：普通退出登录后派生密钥可能留存内存至进程退出（无功能影响）；会话删除传播依赖「下次同步时裁剪」，双端离线互删场景以最后同步端为准。
...
- [x] WiFi 下课程/考试数据变更 ≤30s 自动上云，断网恢复自动补传，开关关闭零上传（P2-7a）——代码侧完成，双实例验收待 U-P2-2
- [x] 双实例双写冲突按 LWW 正确裁决，冲突可在设置页查看日志（P2-7b）——代码侧完成，双实例验收待 U-P2-2
- [x] 课程/考试 ID 维持现状无迁移；新会话表为 String 主键 + 本地 UUID；id-generator 未接入任何调用链（P2-7c）
- [x] 会话双端一致；云端控制台可见记录均为密文；新表 permissions 无 World 角色；登出后不再上行（P2-7d）——代码侧完成，联调依赖 U-P2-1 建表 + U-P2-2 验收
- [x] 截图导入可预览、修正、剔除后再入库（P2-9）——代码侧完成，真图操作验收待 U-P2-3
- [x] 全程：commit 前 build+lint 双绿（lint 0 error / 63 条既有 warning 与基线一致）、分支不 push
